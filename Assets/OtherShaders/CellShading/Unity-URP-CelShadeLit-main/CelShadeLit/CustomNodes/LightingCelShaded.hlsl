...
struct SurfaceVariables {
   ...
   float specScalar;
   EdgeConstants ec;
   float metallic; // Added metallic property
};
...
float3 CalculateCelShading(Light l, SurfaceVariables s) {
   ...
   specular = pow(specular, s.shininess)* s.specScalar;
   
   // Use the metallic property to influence the specular reflection
   float metallicReflection = lerp(0.04f, 0.5f, s.metallic);
   specular *= metallicReflection;
...
void LightingCelShaded_float(float Metallic, float Smoothness, float RimStrength, float RimAmount, 
   float RimThreshold, 
   float3 Position, float3 Normal, float3 View, float EdgeDiffuse,
   float EdgeSpecular, float EdgeDistanceAttenuation,
   float EdgeShadowAttenuation, float EdgeRim, float specScalar, out float3 Color) {
   ...
   s.ec.rim = EdgeRim;
   s.metallic = Metallic; // Set the metallic property
