...
namespace Unity.Scenes
{
    ...
    public class SubScene : MonoBehaviour
    {
        ...
        void OnValidate()
        {
            _SceneGUID = new Hash128(AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(_SceneAsset)));
                ...
                if (_IsAddedToListOfAllSubScenes && IsInMainStage())
                {
                    if (_SceneGUID != _AddedSceneGUID)
                    {
                        foreach (var world in World.All)
                        {
                            ...
                            entityManager.AddComponentData(sceneEntity, new SceneReference { SceneGUID = _SceneGUID });
        ...
        void RemoveSceneEntities()
        {
            if (_AddedSceneGUID != default)
            {
                ...
                {
                    var entityManager = world.EntityManager;
                    var sceneEntityQuery = entityManager.CreateEntityQuery(typeof(SceneReference));
                    var sceneEntities = sceneEntityQuery.ToEntityArray(Allocator.TempJob);

                    foreach (var sceneEntity in sceneEntities)
                    {
                        var sceneReference = entityManager.GetComponentData<SceneReference>(sceneEntity);
                        if (sceneReference.SceneGUID == sceneGUID)
                        {
