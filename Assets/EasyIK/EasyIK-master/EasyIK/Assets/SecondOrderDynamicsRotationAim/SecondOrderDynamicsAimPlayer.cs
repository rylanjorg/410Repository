...
public class SecondOrderDemoAimplayer : MonoBehaviour
{
    public enum axis{
        x,
        y,
    ...
    [TabGroup("fsm", "Inscribed")] public FSMPlayer fsmPlayer;  
    [TabGroup("fsm", "Inscribed")] [Range(0,1)] public float animationBlend;  
    ...
    void Awake()
    {
        ...
        else if(axisToRotate == axis.y)
        {
            ...
            thisTargetRotation = playerForwardVector.eulerAngles.y;
    ...
    void RotateGun()
    {
        thisTargetRotation = rootTransformRotation.eulerAngles.y;
        ...
        {
           playerForwardVector.rotation = Quaternion.AngleAxis(currentPForwardAngle + minAngle, Vector3.up);
            //currentPForwardAngle = playerForwardVector.eulerAngles.y;
        }

        if( relativeAngle > maxAngle)
        {
            playerForwardVector.rotation = Quaternion.AngleAxis(currentPForwardAngle + maxAngle, Vector3.up);
             //currentPForwardAngle = playerForwardVector.eulerAngles.y;
        }*/
    
        thisCurrentRotation =  NormalizeAngle_(transform.eulerAngles.y, lastRotation);
        thisTargetRotation =  NormalizeAngle_(thisTargetRotation, lastRotation);

        if(updateMode == UpdateMode.FixedUpdate) 
        {
            float targetRotation = thisSecondOrderDynamics.Update(Time.fixedDeltaTime, thisTargetRotation,  thisCurrentRotation);
            newYRotation = Mathf.Lerp(thisCurrentRotation, targetRotation, animationBlend);
            if(fsmPlayer.playerData.playerRuntimeData.animationBlend > 1) newYRotation = thisCurrentRotation;

        }
        else if (updateMode == UpdateMode.Update) 
        {
            float targetRotation = thisSecondOrderDynamics.Update(Time.deltaTime, thisTargetRotation,  thisCurrentRotation);
            newYRotation = Mathf.Lerp(thisCurrentRotation, targetRotation, animationBlend);
            if(fsmPlayer.playerData.playerRuntimeData.animationBlend > 1) newYRotation = thisCurrentRotation;
        }
