...
public partial struct SceneInitializationSystem : ISystem
{
    ...
    {
        // Game init
        if (SystemAPI.HasSingleton<GameResources>())
        {
            ref GameResources sceneInitializer = ref SystemAPI.GetSingletonRW<GameResources>().ValueRW;
            

            if(!sceneInitializer.isInitalized)
            {
                // Cursor
                ...
                //state.EntityManager.DestroyEntity(SystemAPI.GetSingletonEntity<GameResources>());
                sceneInitializer.isInitalized = true;
            }
